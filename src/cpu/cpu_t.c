...
int cpu_init(struct cpu_t *cpu, uint8_t **code_p)
{
	...
	char *newmemory = calloc(4096, sizeof(uint8_t));
...
{
	cpu_newrip_write_byte(X86_64_OPERAND_SIZE);
	cpu_newrip_write_byte(cpu_rip_byte(cpu) + 0xb8);
	cpu_newrip_write_qword(cpu_rip_qword(cpu));
}
